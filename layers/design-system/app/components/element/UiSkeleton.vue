...
const {
  ...
  /**
   * Skeleton shape. `text`/`bar`/`circle` size themselves from `base`/`index`;
   * `block` sets no width or height of its own, so the caller's utility classes
   * decide the box — use it whenever the placeholder must match a known layout
   * (a `w-full` row, a fixed-width label). The self-sizing types write an inline
   * `width`/`height`, which silently beats any `w-*`/`h-*` class passed in.
   */
  type?: 'text' | 'bar' | 'circle' | 'block'
...
<template>
  ...
  <div v-if="type === 'text' && lines > 1" aria-hidden="true" v-bind="attrs" class="flex flex-col gap-2">
    <div
      v-for="i in lines"
      :key="i"
      class="ui-skeleton rounded h-3 max-w-full"
  ...
  <!-- Caller-sized box: no inline width/height, so `w-*`/`h-*` classes win -->
  <div
    v-else-if="type === 'block'"
    aria-hidden="true"
    v-bind="attrs"
    class="ui-skeleton rounded"
  />
