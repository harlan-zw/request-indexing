...
const { status = 'neutral', icon, label, size = 'sm', prominent = false, readable = false } = defineProps<{
  ...
  prominent?: boolean
  /** Keep a report-visible inline label at the 14px readable tier. */
  readable?: boolean
}>()
...
<template>
  ...
  <div v-if="size === 'md'" class="flex flex-col items-center gap-2">
    <div class="size-12 rounded-2xl flex items-center justify-center" :class="colors.bg">
      <UiIcon v-if="icon" :name="icon" class="size-6" :class="colors.text" aria-hidden="true" />
    </div>
    <slot>
  ...
  <span v-else class="inline-flex items-center gap-1.5 whitespace-nowrap" :class="readable ? 'text-sm' : 'text-xs'">
