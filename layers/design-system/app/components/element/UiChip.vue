...
<template>
  ...
  >
    <UiIcon v-if="icon" :name="icon" class="size-3 shrink-0" aria-hidden="true" />
    <slot />
    <button
      v-if="removable"
      type="button"
      class="-mr-0.5 inline-flex cursor-pointer items-center justify-center rounded-sm opacity-60 transition-opacity hover:opacity-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary"
