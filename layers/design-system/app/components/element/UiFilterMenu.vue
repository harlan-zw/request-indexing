...
import { ToggleGroupItem, ToggleGroupRoot, UiIcon, UiMetricLabel, UInput, UiPopover } from '#components'
...
const {
  ...
  /** Show the Saved Filters section (header + Save current + list). Disable on
   *  surfaces that don't implement saved filters so no dead "Save current"
   *  button renders. */
  showSaved?: boolean
}>()
      ...
      <div class="w-[270px] py-1.5">
        ...
        <template v-if="activeDimension">
          <button
            ...
            class="cursor-pointer w-full flex items-center gap-1.5 px-3 py-2.5 sm:py-1 text-xs font-medium text-muted hover:text-default transition-colors outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-inset"
            @click="closeDimension"
          >
            <UiIcon name="chevron-left" class="size-3.5 shrink-0" aria-hidden="true" />
        ...
        <template v-else>
          <!-- Active-filter header + one-click clear (only when something is applied). -->
          <div v-if="activeCount" class="px-3 pt-0.5 pb-1.5 flex items-center justify-between">
            ...
            <button
              ...
              class="min-h-11 cursor-pointer rounded text-mini text-muted outline-none hover:text-default focus-visible:ring-2 focus-visible:ring-primary sm:min-h-0"
              @click="emit('clear')"
            >
              Clear all
            </button>
          ...
          <div v-if="dimensions.length" ref="dimensionsRoot" role="group" aria-label="Filter dimensions">
            <button
              ...
              class="cursor-pointer w-full flex items-center gap-2 px-3 py-2.5 sm:py-1 text-xs text-muted hover:text-default hover:bg-[var(--ui-bg-elevated)]/50 transition-colors outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-inset"
          ...
          <template v-if="showSaved">
            ...
            <div v-if="savedFilters.length" role="group" aria-label="Saved filters">
              <div
                ...
                :key="s.id"
                class="group w-full flex items-center gap-2 px-3 py-2.5 sm:py-1 text-xs text-muted hover:bg-[var(--ui-bg-elevated)]/50 transition-colors"
              >
                <button
                  type="button"
          ...
          <template v-if="presetFilters.length">
            ...
            <div role="group" aria-label="Preset filters">
              <button
                ...
                class="relative flex min-h-11 w-full cursor-pointer items-center gap-2 px-3 py-2.5 text-xs outline-none transition-colors focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-inset sm:min-h-0 sm:py-1"
              ...
              >
                <span
                  ...
                  aria-hidden="true"
                />
                <UiIcon :name="p.icon ?? 'ai'" class="size-3.5 shrink-0 text-dimmed" aria-hidden="true" />
