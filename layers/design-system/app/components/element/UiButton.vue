...
import { useMediaQuery, useMounted, useMouseInElement } from '@vueuse/core'
...
import { UButton, UiIcon, UiSlotText } from '#components'
...
type MotionButtonProps = Omit<ButtonProps, 'color' | 'variant' | 'icon' | 'leadingIcon' | 'trailingIcon' | 'onClick'> & MotionExtras
...
// Mount-gated: `canHover` is a media query, so it's false on the server and
// flips true at hydration on desktop — rendering the v-if FX spans straight
// from `canHover` mismatches on every desktop pageview. The FX layers are all
// hover-gated (invisible at rest), so deferring them to post-mount is free.
const isMounted = useMounted()
const showFx = computed(() => isMounted.value && canHover.value && intensity.value !== 'subtle' && !isDisabled.value)
...
const lift = computed(() => {
  ...
  if (reduced.value || !canHover.value || purpose === 'link')
...
const wrapperStyle = computed(() => {
  // Hover capability is only authoritative in the browser. Keep the default
  // FX variables deterministic through SSR/hydration, then let the mouse
  // tracker update them; subtle/disabled controls never consume the values.
  if (intensity.value === 'subtle' || isDisabled.value)
    return undefined
  return {
    '--mb-mx': `${mxPct.value}%`,
    '--mb-my': `${myPct.value}%`,
...
// `useReducedMotion()` is browser-only knowledge. Gate the initial pose on the
// same mounted state as the hover FX so SSR and hydration both render the icon
// at rest. Later keyed icon changes can still animate on motion-enabled clients.
const leadingInitial = computed(() => isMounted.value && !reduced.value
...
const shouldAutoAnimateLabel = computed(() => {
  if (!buttonLabel.value || slots.default || block || loading || animatedLabel === false)
    return false
    ...
    >
      ...
      >
        <UButton
          ...
          class="ui-motion-button__btn relative z-10 whitespace-nowrap" :class="[{ 'hover:underline underline-offset-2': purpose === 'link' }]"
...
/* Callers may opt into a pill. Every motion layer must follow that shape. */
.ui-motion-button:has(.ui-motion-button__btn.rounded-full) {
