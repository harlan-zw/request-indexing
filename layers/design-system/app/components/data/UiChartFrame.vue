...
import { ClientOnly, UiChartAnnotations, UiChartBrushOverlay, UiChartTooltip } from '#components'
...
const emit = defineEmits<{
  /** Forwarded from the marker overlay: a user-editable annotation was activated. */
...
<template>
  <ClientOnly>
    <UiChartTooltip
      :x="cardSpring"
      :y="cursorYSpring"
      :visible="tooltipVisible && !annotationInteracting"
      :placement="placement"
      :label="hoverLabel"
    >
      ...
      <div v-if="tooltipAnnotations.length" class="mt-1.5 pt-1.5 border-t border-default space-y-1">
        <div v-for="ann in tooltipAnnotations" :key="ann.id" class="grid grid-cols-[auto_minmax(0,1fr)] gap-x-1.5">
          <span :class="ann.toneClass" class="size-1.5 rounded-full shrink-0 mt-1" aria-hidden="true" />
          <div class="min-w-0">
            <p class="text-sm font-medium text-default truncate">
              {{ ann.label }}
            </p>
            <p v-if="ann.description" class="text-sm text-muted mt-0.5">
              {{ ann.description }}
            </p>
          </div>
        </div>
