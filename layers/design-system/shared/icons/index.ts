 ...
 *
 * Source scanning is disabled in the layer config. `iconBundleList()` is the
 * explicit active-set client bundle, so candidate-set literals stay build-only.
 */
 ...
 */
export const ACTIVE_ICON_SET: IconSet = 'hugeicons'

/** A curated semantic role or registered synonym. Prefer this for static registries. */
export type IconAlias = keyof typeof ICON_ALIASES
export type SemanticUiIcon = IconRole | IconAlias

/**
 * Component-facing icon input. Runtime provider/flag data can be dynamic, while
 * literal role names still retain autocomplete through `SemanticUiIcon`.
 */
export type RawUiIcon = `i-${string}`
export type UiIcon = SemanticUiIcon | RawUiIcon | (string & {})
...
// Brand marks used by UiSourceLogos. Keep these local in every app that extends
// the design system, so Source identity never depends on Iconify at runtime.
const SOURCE_LOGO_ICONS = [
