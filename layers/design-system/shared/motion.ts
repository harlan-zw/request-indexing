...
export const liftPresets: Record<LiftIntensity, LiftPreset> = {
  ...
  /** Restrained lift with a touch of spring: the standard. */
  default: {
    hover: { scale: 1.025, y: -2 },
    tap: { scale: 0.98, y: 0 },
    transition: { type: 'spring', stiffness: 320, damping: 21, mass: 0.8 },
  },
  /** Emphatic lift: visible overshoot with a controlled settle. */
  cta: {
    hover: { scale: 1.045, y: -4 },
    tap: { scale: 0.96, y: 0 },
    transition: { type: 'spring', stiffness: 240, damping: 15, mass: 1.1 },
  },
}
