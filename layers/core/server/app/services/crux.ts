...
export async function fetchCrux(domain: string, formFactor: 'PHONE' | 'TABLET' | 'DESKTOP' = 'PHONE', path: string | null = null) {
  const origin = withTrailingSlash(withHttps(domain))
  const body: { formFactor: string, url?: string, origin?: string } = {
...
function normaliseCruxHistory(data: CrUXHistoryResult, formFactor: string, isOrigin = true): NormalizedCrUXHistoryResult {
  ...
  const dates = data.collectionPeriods.map(period => format(new Date(period.firstDate.year, period.firstDate.month, period.firstDate.day), 'yyyy-MM-dd'))
  function normaliseP75(segment: any, i: number) {
