/**
 * Builds a `/dashboard/site/:id/...` link from a fleet row's public id.
 *
 ...
 * `/dashboard/site/undefined/...`. Routing every dashboard site link through
 * this function means that typo can only happen once.
 */
export function dashboardSiteHref(siteId: string, path: string): string {
  return `/dashboard/site/${encodeURIComponent(siteId)}/${path}`
}
