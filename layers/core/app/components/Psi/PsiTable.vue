...
import type { GoogleSearchConsoleSite, GscDataRow } from '~~/layers/core/app/types/data'
import { callFnSyncToggleRef } from '~~/layers/design-system/composables/loader'
import { exponentialMovingAverage } from '~~/layers/core/app/utils/time-smoothing/exponentialMovingAverage'
import { simpleMovingAverage } from '~~/layers/core/app/utils/time-smoothing/simpleMovingAverage'
    ...
    <TableData :value="value" :columns="columns" :filters="filters" @update:expanded="updateExpandedData">
      <template #keyword-data="{ row, rows, expanded }">
        <div class="flex items-center">
          <div class="relative group w-[225px] truncate text-ellipsis">
            <div class="flex items-center">
              <UButton class="max-w-[185px] block" variant="link" size="xs" :class="mock ? ['pointer-events-none'] : []" color="neutral" @click="q = row.keyword">
              ...
              <UBadge v-else-if="row.lost" size="xs" color="error" variant="subtle">
                Lost
              </UBadge>
            </div>
            <UProgress :value="Math.round((row.clicks / highestRowClickCount(rows)) * 100)" color="info" size="xs" class="ml-2 opacity-75 group-hover:opacity-100 transition" />
          </div>
        </div>
      ...
      <template #page-data="{ row, expanded }">
        <div v-if="!expanded" class="flex items-center">
          <UButton :title="row.page" variant="link" size="xs" :class="mock ? ['pointer-events-none'] : []" :to="`/dashboard/site/${site.domain}/pages?q=${encodeURIComponent(row.page)}`" color="neutral">
        ...
        <div v-else>
          <ul class="space-y-2">
            <li v-for="(p, key) in expandedRowData?.pages || []" :key="key">
              <UButton :title="p.page" variant="link" size="xs" :class="mock ? ['pointer-events-none'] : []" :to="`/dashboard/site/${site.domain}/pages?q=${encodeURIComponent(p.page)}`" color="neutral">
                <div>
                  <div class="max-w-[150px] truncate text-ellipsis">
                    {{ p.page }}
                  </div>
