...
function fmtDistance(d: string | number | Date | undefined | null) {
  if (!d)
    return ''
...
function hasOneHourPassed(date?: string | number) {
  if (!date)
    return true
  return differenceInHours(new Date(), new Date(date)) >= 1
}
  ...
  <TableData :columns="columns" :value="value" :filters="mock ? [] : filters" @update:rows="rows => visibleRows = rows">
    <template #url-data="{ row }">
      <div style="max-width: 400px;" class="flex flex-col">
        <UButton :title="row.url" variant="link" size="xs" :class="mock ? ['pointer-events-none'] : []" :to="joinURL(`https://${siteUrlFriendly}`, row.url)" target="_blank" color="neutral" class="w-full">
        ...
        <UTooltip v-if="getUpdatedRow(row)?.inspectionResult?.inspectionResultLink" mode="hover" text="View Inspection Result">
          <UButton size="xs" target="_blank" :to="mock ? undefined : getUpdatedRow(row)?.inspectionResult?.inspectionResultLink" icon="i-heroicons-document-magnifying-glass" color="neutral" variant="link">
    ...
    <template #inspection-data="{ row }">
      <div>
          ...
          <template v-if="getUrlNotificationLatestUpdate(row)?.type === 'URL_UPDATED' || getUpdatedRow(row)?.inspectionResult?.indexStatusResult?.verdict === 'NEUTRAL'">
            ...
            <div class="flex items-center justify-between">
              <div class="text-gray-600">
                ...
                {{ fmtDistance(getUpdatedRow(row)?.lastInspected) }}
              </div>
              <div v-if="getUpdatedRow(row)?.lastInspected && !hasOneHourPassed(row?.lastInspected)">
                <UButton :disabled="mock" color="neutral" size="xs" class="mt-2" icon="i-heroicons-arrow-path" :loading="inspectionsLoading.includes(row.url)" @click="inspectUrl(row)">
                  Inspect Again
                </UButton>
              </div>
              <div v-else>
        ...
        <div v-if="!getUpdatedRow(row)?.lastInspected">
          <UButton size="xs" color="neutral" :loading="inspectionsLoading.includes(row.url)" @click="inspectUrl(row)">
            Inspect
          </UButton>
        </div>
