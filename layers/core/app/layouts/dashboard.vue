...
const site = computed(() => {
  ...
  if (!slug)
    return undefined
  return sites.value.find(s => String(s.siteId) === slug)
})
...
const domains = computed(() => {
  if (!site.value)
    return []
  return sites.value.filter(s => s.property === site.value!.property)
})
...
<template>
  <UDashboardGroup>
    ...
    >
      <template #header>
        <UDropdownMenu v-if="site" :items="siteSwitcherItems" :content="{ align: 'start' }" class="w-full">
      ...
      <template v-if="site">
        <UButton icon="i-ph-arrow-u-down-left-fill" color="neutral" variant="link" size="xs" to="/dashboard" class="self-start">
          Back
        </UButton>
        ...
        <div class="space-y-4">
          <div>
            <div class="px-2 text-xs font-semibold text-muted uppercase tracking-wider mb-1">
              Dashboards
            </div>
    ...
    <UDashboardPanel id="dashboard-content">
      <template #header>
        <DashboardHeader>
          <div class="flex items-center gap-2 text-lg font-semibold text-highlighted">
            <UIcon v-if="$route.meta.icon" :name="($route.meta.icon as string)" />
            {{ $route.meta.subTitle || $route.meta.title }}
          </div>
        </DashboardHeader>
      </template>
      <template #body>
        <div class="mx-auto mb-10 w-full max-w-6xl">
          <slot />
        </div>
      </template>
    </UDashboardPanel>
