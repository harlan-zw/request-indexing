...
onMounted(() => {
  ...
  const areaSeries = _chart.addBaselineSeries({
    ...
    priceScaleId: 'right',
    baseValue: { type: 'price', price: 800 },
  ...
  <div ref="container" class="w-full h-full">
    ...
    <div class="tooltip">
      <div v-if="tooltipData.time" class="dark:text-gray-200 text-gray-600 text-xs">
        ...
        <div class="font-bold" :class="tooltipData.lcp >= 1800 ? 'text-red-500' : tooltipData.lcp >= 800 ? 'text-yellow-500' : 'text-green-500'">
          {{ useHumanMs(tooltipData.lcp) }}
        </div>
      </div>
    </div>
  </div>
