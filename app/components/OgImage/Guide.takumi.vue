...
const themeRgb = computed(() => '34, 197, 94') // green-500
</script>
...
<template>
  ...
  >
    ...
    <div class="flex flex-row justify-center items-center text-left w-full mb-12">
      <svg height="80" width="80" class="text-green-500" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 267 262" xml:space="preserve">
      ...
      <div class="text-black-800 text-4xl text-mono font-bold ml-8" style="letter-spacing: -2px;">
        Request Indexing
      </div>
    </div>
    <div class="flex flex-col w-full items-center text-center relative">
      <div v-if="headline" class="text-green-500 text-lg font-medium mb-3">
        {{ headline }}
      </div>
      <div class="text-4xl font-bold mb-4" style="line-height: 1.3">
        {{ title }}
      </div>
      <div v-if="description" class="text-xl text-gray-500 max-w-[600px]" style="line-height: 1.5">
        {{ description }}
      </div>
    </div>
  </div>
