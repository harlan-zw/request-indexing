...
const dashboards = computed(() => !site.value
  ...
  : [
      ...
      {
        label: 'Analysis',
        to: joinURL('/dashboard/site', encodeURIComponent(site.value.siteId), 'analysis'),
        icon: 'i-ph-chart-pie-slice-duotone',
      },
      {
        label: 'Sitemaps',
        to: joinURL('/dashboard/site', encodeURIComponent(site.value.siteId), 'sitemaps'),
...
<template>
  <UDashboardLayout class="mx-auto max-w-screen-2xl">
    <UDashboardPanel :width="250" :resizable="{ min: 200, max: 400 }" collapsible>
      <div class="h-[--ui-header-height] flex-shrink-0 flex justify-center flex-col items-center border-b border-default px-4 gap-x-4 min-w-0">
        <template v-if="site">
          <USelectMenu v-if="site" :model-value="site" variant="none" :options="sites" value-attribute="siteId" class="w-full" @change="changeSite">
            ...
            <template #default="{ open }">
              <UButton color="neutral" variant="link" class="flex items-center gap-1 w-full flex-grow" :ui="{ padding: { xl: 'pl-0 ' } }">
        ...
        <USelectMenu v-else v-model="session.team.name" variant="none" :options="[{ label: session.team.name }, { label: 'Create Team', icon: 'i-heroicons-plus' }]">
          <template #default="{ open }">
            <UButton color="neutral" variant="link" size="sm" class="flex items-center gap-2">
              {{ session.team.name }}
              <UIcon name="i-heroicons-chevron-right-20-solid" class="w-5 h-5 transition-transform text-gray-400 dark:text-gray-500" :class="[open && 'transform rotate-90']" />
            </UButton>
          </template>
      ...
      <UDashboardSidebar>
        <template v-if="site">
          <UButton icon="i-ph-arrow-u-down-left-fill" color="neutral" variant="link" size="xs" to="/dashboard">
            Back
          </UButton>
        ...
        <template #footer>
          <div>
            <ULink to="/dashboard" color="neutral" size="xs" class="flex items-center gap-2 group mr-10">
      ...
      <UDashboardPanel grow>
        <DashboardHeader>
          <USelectMenu size="xs" :options="siteLinks">
            <template #default="{ open }">
              <UButton color="neutral" variant="ghost" :class="[open && 'bg-gray-50 dark:bg-gray-800']" trailing-icon="i-heroicons-chevron-down-20-solid" @click="open">
                <div class="flex items-center gap-2">
