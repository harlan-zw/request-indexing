...
import type { JobSelect, UserSelect } from '#shared/types/database'
    ...
    <TableData :value="data" :columns="columns" :filters="filters">
      ...
      <template #response-data="{ row }">
        <div v-if="row.response">
          {{ row.response }}
          <!--          <div>{{ parse(row.response).status }}</div> -->
          <!--          <pre>{{ parse(row.response).body }}</pre> -->
        </div>
      </template>
      ...
      <template #actions-data="{ row }">
        <UButton size="xs" color="neutral" @click="retry(row)">
          Retry
        </UButton>
      </template>
    </TableData>
