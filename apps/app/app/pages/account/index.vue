...
async function revokeIndexingAuth() {
  ...
  await $fetch('/api/indexing/auth', {
    ...
    .then(() => {
      setTimeout(() => {
        toast.add({ title: 'Google Token Revoked', description: 'You have revoked access to the Web Indexing API.', color: 'success' })
        fetch()
      }, 500) // make sure user knows we actually did something (it's too quick)
    })
    .catch(() => {
      toast.add({ title: 'Failed to revoke Google Token', description: 'Sorry about that, maybe try again later.', color: 'error' })
    })
}
...
function deleteAccount() {
  ...
  $fetch('/api/user/me', {
    ...
    setTimeout(() => {
      toast.add({ id: 'logout', title: 'Account Deleted', description: 'Your account has been deleted.', color: 'success' })
      isDeletingAccount.value = false
      session.value = null
      logout()
    }, 500) // make sure user knows we actually did something (it's too quick)
  }).catch(() => {
    toast.add({ title: 'Failed to delete account', description: 'Sorry about that, maybe try again later.', color: 'error' })
  })
}
  ...
  <div>
    <div class="mb-8">
      <div class="mb-5">
        <h2 class="text-lg font-bold flex items-center gap-1 mb-1">
          <UIcon name="i-ph-key-duotone" class="mr-1.5" />
          API Key
        </h2>
      </div>
      <ProAccountApiKey />
    ...
    <div>
      ...
      <template v-if="user.indexingOAuthIdNext">
        ...
        <UButton color="error" variant="soft" :loading="isRevokingIndexingAuth" @click="revokeIndexingAuth">
          Revoke Tokens
        </UButton>
      </template>
      <template v-else>
    ...
    <div>
      ...
      <UButton color="error" variant="outline" :loading="isDeletingAccount" @click="confirmDeleteAccount = true">
        Delete Account
      </UButton>
      <UModal v-model="confirmDeleteAccount">
        <div class="p-4">
          ...
          <UButton color="error" variant="outline" :loading="isDeletingAccount" @click="deleteAccount">
            Delete Account
          </UButton>
        </div>
      </UModal>
