...
CREATE TABLE `site_date_analytics` (
	...
	`mobile_origin_cls_75` integer,
	`mobile_origin_ttfb_75` integer,
	`mobile_origin_fcp_75` integer,
	`mobile_origin_lcp_75` integer,
	`mobile_origin_inp_75` integer,
	`desktop_origin_cls_75` integer,
	`desktop_origin_ttfb_75` integer,
	`desktop_origin_fcp_75` integer,
	`desktop_origin_lcp_75` integer,
	`desktop_origin_inp_75` integer,
	`origin_loading_experience` text,
	`keywords` integer,
	`pages` integer,
...
--> statement-breakpoint
CREATE TABLE `site_pagespeed_insight_scan_audits` (
	`site_pagespeed_insight_scan_audit_id` integer PRIMARY KEY NOT NULL,
	`site_pagespeed_insight_scan_id` integer NOT NULL,
	...
	`created_at` integer DEFAULT (CURRENT_TIMESTAMP) NOT NULL,
	`updated_at` integer DEFAULT (CURRENT_TIMESTAMP) NOT NULL,
	FOREIGN KEY (`site_pagespeed_insight_scan_id`) REFERENCES `site_pagespeed_insight_scans`(`site_pagespeed_insight_scan_id`) ON UPDATE no action ON DELETE no action
);
--> statement-breakpoint
...
CREATE INDEX `audit_idx` ON `site_pagespeed_insight_scan_audits` (`audit_id`);--> statement-breakpoint
