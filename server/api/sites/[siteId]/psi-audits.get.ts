...
export default defineEventHandler(async (e) => {
  ...
  const { filters, offset, q, sort, pageSize } = getQueryAsyncDataTable<
    'accessibility' | 'performance' | 'best-practices' | 'seo'
  >(e)
  const where = []
  if (filters.includes('accessibility'))
    where.push(eq(sitePageSpeedInsightScanAudits.category, 'accessibility'))
  else if (filters.includes('performance'))
    where.push(eq(sitePageSpeedInsightScanAudits.category, 'performance'))
  else if (filters.includes('best-practices'))
    where.push(eq(sitePageSpeedInsightScanAudits.category, 'best-practices'))
  else if (filters.includes('seo'))
    where.push(eq(sitePageSpeedInsightScanAudits.category, 'seo'))

  const db = useDrizzle()

  return {
    rows: await db.select({
      auditId: sitePageSpeedInsightScanAudits.auditId,
      category: sitePageSpeedInsightScanAudits.category,
      score: avg(sitePageSpeedInsightScanAudits.score).as('score'),
      weight: max(sitePageSpeedInsightScanAudits.weight).as('weight'),
