...
<template>
  <TableAsyncData :key="filter" :filter="filter" :sort="sort" :pagination="pagination" :searchable="searchable" :page-size="pageSize" :path="`/api/sites/${site.siteId}/countries`" :columns="columns" :filters="filters" :expandable="expandable" @page-change="p => page = p" @update:expanded="updateExpandedData">
    ...
    <template #country-data="{ row }">
      <div class="flex-1 flex-grow min-h-full">
        <ProgressPercent :value="row.percent" :total="100" class="min-w-full">
          <NuxtLink :href="`/dashboard/site/${site.siteId}/countries/${encodeURIComponent(row.country)}`" :title="`Open ${row.path}`" class="max-w-[260px] flex items-center justify-between transition py-1 rounded text-xs hover:bg-gray-100 block" color="gray">
            <div class="flex items-center gap-2 mb-1">
              <Icon :name="`circle-flags:${row.countryCode.toLowerCase()}`" />
              <div class="capitalize text-black max-w-[260px] truncate text-ellipsis">
                {{ row.country }}
              </div>
            </div>
          </NuxtLink>
